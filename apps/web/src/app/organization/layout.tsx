...
import { OrganizationSidebarNav } from "./_components/OrganizationSidebarNav";
...
export default async function OrganizationLayout({
  ...
  if (me?.role !== "ADMIN" || !me.organizationId) {
    return (
      <AccessDenied
    ...
    <div className="flex min-h-screen bg-slate-50">
      <aside className="sticky top-0 flex h-screen w-64 shrink-0 flex-col border-r border-slate-800/50 bg-[#0F172A] text-slate-100">
        <div className="flex items-center gap-2.5 px-6 pt-6">
          <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-emerald-600 text-sm font-bold text-white shadow-md shadow-emerald-600/30">
    ...
    <div className="flex min-h-screen items-center justify-center bg-slate-50 px-6">
      <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-8 shadow-card">
        ...
        <p className="mt-2 text-sm text-slate-600">
          Organization management is restricted to ADMIN accounts assigned to an
          organization.
        </p>
        ...
        <div className="mt-6 flex flex-col gap-3">
          {currentRole === "SUPER_ADMIN" ? (
            ...
            <Link
              ...
              className="inline-flex items-center justify-between rounded-xl bg-emerald-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-emerald-700"
            >
